...
class TestCreateCourier:
    ...
    def test_create_identical_courier(self, create_courier):
        courier = create_courier['payload']
        with allure.step('Создание курьера с повторяющимся логином'):
            response = requests.post(f'{Url.BASE_URL}{Url.CREATE_COURIER_URL}', data = courier)
